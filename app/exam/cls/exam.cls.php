...
class exam_exam
{
	...
	//参数：批量试题内容字符串，章节ID
	//返回值：true
	public function importQuestionBat($uploadfile,$tknowsid,$questionparent = 0,$_user)
	{
		// $this->_user
		...
		while ($data = fgetcsv($handle))
		{
			...
			if(count($question) >= 6)
			{
				...
				$args['questionselect'] = $this->ev->addSlashes(htmlspecialchars('<p>'.$this->ev->addSomeStr($args['questionselect'], "|", "</p><p>").'</p>'));
				// $args['questionselectnumber'] = intval(trim($question[3]," \n\t"));
				$args['questionselectnumber'] = count(array_filter(explode("|", $args['questionselect'])));
				$args['questionanswer'] = str_replace('|', '', $this->ev->addSlashes(htmlspecialchars(iconv("GBK","UTF-8//IGNORE",trim($question[3]," \n\t")))));
				$args['questiondescribe'] = $this->ev->addSlashes(htmlspecialchars(iconv("GBK","UTF-8//IGNORE",trim($question[4]," \n\t"))));
				$args['questiondescribe'] = $this->ev->addSlashes(htmlspecialchars('<p>'.$this->ev->addSomeStr($args['questiondescribe'], "|", "</p><p>").'</p>'));
				if(!$tknowsid)
				$questionknowsid = trim($question[5]," \n\t");
				else
				...
				$args['questionlevel'] = intval(trim($question[6]," \n\t"));
				$args['questioncreatetime'] = TIME;
				$args['questionuserid'] = $_user['sessionuserid'];
				$args['questionusername'] = $_user['sessionusername'];
