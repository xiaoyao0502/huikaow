...
class app
{
	...
	public function basic()
	{
		...
		switch($subaction)
		{
			...
			if($this->ev->get('insertknows'))
			{
				...
				$errmsg = trim($errmsg,' ,');
				if($errmsg)$errmsg = $errmsg.'等章节已经存在，程序自动忽略！';
				$message = array(
					'statusCode' => 200,
					"message" => "操作成功！".$errmsg,
			...
			$message = array(
				'statusCode' => 300,
				"message" => "操作失败，请删除该课程下所有科目后再删除本课程"
			);
			else
			{
	...
	public function questions()
	{
		...
		{
			$this->tpl->assign('search',$search);
			if($search['questiontype'] == 8) { //综合题
				$search['questiontype_child'] = 1;
				$search['questiontype'] = 0;
			}elseif($search['questiontype'] == 9) { //组合选择题
				$search['questiontype_child'] = 2;
				$search['questiontype'] = 0;
			}elseif($search['questiontype'] == 10) { //案例分析题
				$search['questiontype_child'] = 3;
				$search['questiontype'] = 0;
			}elseif($search['questiontype'] == 11) { //计算分析题
				$search['questiontype_child'] = 4;
				$search['questiontype'] = 0;
			}
			foreach($search as $key => $arg)
			{
		...
		switch($subaction)
		{
			...
			if($this->ev->get('insertquestion'))
			{
				...
				$this->exam->importQuestionBat($uploadfile,$knowsid,0,$this->_user);
				$message = array(
					'statusCode' => 200,
					"message" => "操作成功",
					"callbackType" => "forward",
			...
			if($search['questiontype_child'])
			{
				$args[] = array("AND","questions.questiontype_child = :questiontype_child",'questiontype_child',$search['questiontype_child']);
			}
	...
	public function exams()
	{
	    ...
	    switch($subaction)
	    {
	       ...
	       default:
	                ...
	                $exams = $this->exam->getExamSettingList($page,10,$args);
	                $subjects = $this->basic->getSubjectList();
	                $knowsList = $this->basic->getKnowsList();
	                $this->tpl->assign('subjects',$subjects);
	                $this->tpl->assign('knowsList',$knowsList);
	                $this->tpl->assign('exams',$exams);
